...
namespace BrightExistence.SimpleTools
{
    ...
    public class SimpleResearchable : IResearchable
    {
        ...
        public SimpleResearchable(string strName, string strNAMESPACE = BrightExistence.ColonyShop.Data.NAMESPACE)
        {
            if (strName != null && strName.Length > 0) Name = strName;
            NAMESPACE = strNAMESPACE;
            Variables.Researchables.Add(this);
            Pipliz.Log.Write("{0}: Initialized Researchable {1} (it is not yet registered.)", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
        }
        ...
        public void Register ()
        {
            if (enabled)
            {
                ...
                foreach (ItemShell I in IterationRequirements)
                {
                    ...
                    {
                        Pipliz.Log.WriteError("{0} CRITICAL ERROR: SimpleResearchable {1} cannot register properly because 'Variables.itemsMaster' is still null.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
                    }
                    else
                    {
                        Pipliz.Log.Write("{0}: Converting shell references in researchable {1} to InventoryItem objects.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
                        ...
                        {
                            Pipliz.Log.Write("{0} Researchable {1} was given an item key '{2}' as an iteration requirement which was not found by the server.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name, I.strItemkey);
                        }
                    }
                }

                ScienceManager.RegisterResearchable(this);
                Pipliz.Log.Write("{0}: Researchable {1} has been registered with the ScienceManager.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
            }
            else
            {
                Pipliz.Log.Write("{0}: Research {1} has been disabled, and will NOT be registered.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
            }
        }
        ...
        public void addRequirement(string itemKey, int amount = 1)
        {
            ...
            {
                Pipliz.Log.Write("{0}: Research {1} was given a null or invalid item key.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
            }
            else
            {
        ...
        public void addRequirement(SimpleItem requiredItem, int amount = 1)
        {
            ...
            {
                Pipliz.Log.Write("{0}: Research {1} was given a null or invalid SimpleItem object.", BrightExistence.ColonyShop.Data.NAMESPACE, this.Name);
            }
            else
            {
        ...
        public class Unlock
        {
            ...
            public Unlock(string strRecipeName, string strLimitType = null)
            {
                ...
                {
                    throw new ArgumentException("{0}: strRecipeName parameter for Unlock object may not be left null.", BrightExistence.ColonyShop.Data.NAMESPACE);
                }
            }
            ...
            public Unlock(SimpleRecipe unlockMe)
            {
                if (unlockMe == null)
                {
                    throw new ArgumentException("{0}: Unlock constructor argument unlockMe may not be null.", BrightExistence.ColonyShop.Data.NAMESPACE);
                }
                else
                {
